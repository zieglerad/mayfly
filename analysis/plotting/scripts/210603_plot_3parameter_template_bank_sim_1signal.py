...
selected_results = result[
                            ...
                            (result['h_z'] == template_z)
                         ] 

print(selected_results[['h_pa', 'h_E', 'h_z', 'h_r']])

N_pa = selected_results['h_pa'].unique().size
N_E = selected_results['h_E'].unique().size

plot_grid = np.zeros((N_E, N_pa))

for irow, E in enumerate(selected_results['h_E'].unique()):
    for icol, pa in enumerate(selected_results['h_pa'].unique()):
        plot_grid[irow, icol] = selected_results[
                                                (selected_results['h_E'] == E) &
                                                (selected_results['h_pa'] == pa)
                                                ]['T'] 

fig = plt.figure(figsize=(10,8))
ax1 = plt.subplot(111)

pa0 = selected_results['h_pa'].unique()[0]
pa1 = selected_results['h_pa'].unique()[-1]

E0 = selected_results['h_E'].unique()[0]
E1 = selected_results['h_E'].unique()[-1]
img = ax1.imshow(
                plot_grid, 
...
ax1.set_title('MF Scores for a Randomly Generated, Off-grid Signal', size=14)
