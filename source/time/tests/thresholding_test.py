...
threshold=.6
high_power_pix=np.where(mean_y>threshold*np.max(mean_y))
threshold_image=np.zeros(mean_y.shape)
threshold_image[high_power_pix]=1
...
#    plt.title('FFT y')
    plt.figure()
    plt.imshow(threshold_image,origin='lower',cmap='binary')
